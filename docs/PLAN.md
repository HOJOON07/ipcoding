...
| 페이즈 | 목표 | 기간(감) | 산출물 |
|---|---|---|---|
| Phase 0 | 엔진·모델 데이터 기반 확정 | 1–2주 | 벤치마크 리포트, 확정 스택, 시드 사전 v0, 시스템 프롬프트 v1 |
...
| 0.5 | LLM 벤치: Ollama로 후보 4종+참고 1종, 교정 성공률·의도 보존·정돈 품질·TTFT/총지연 | `results_llm.csv` |
| 0.6 | 시스템 프롬프트 개선 반복 (실패 사례 기반) | 프롬프트 v1 |
...
| 2.3 | PromptBuilder: 시스템 프롬프트 v1 + 사전 주입 | 1.6 | Phase 0 산출물 |
...
**리스크**: 소형 LLM이 규칙 4(입력에 응답 금지)를 어기는 사례 — 2.3에서 출력 정제 휴리스틱으로 방어, 빈도 높으면 프롬프트 v2.
