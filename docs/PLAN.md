...
| # | 태스크 | 비고 |
|---|---|---|
| 3.1 | 온보딩 플로우: 권한 3종 안내 + 상태 감지 + 딥링크 (TDD §4) | 최대 이탈 지점 — 문구·흐름에 공들일 것 |
| 3.2 | ModelManager 완성: 다운로드·진행률·sha256·이어받기·재다운로드 | 모델 업그레이드는 앱 릴리스에 태운다(버전별 모델 스펙 고정 → Sparkle 업데이트 → 새 모델 다운로드·검증·구 모델 삭제). 원격 카탈로그(앱 업데이트 없는 모델 교체)는 도입하지 않음 — 모델 교체는 프롬프트·파라미터 벤치 재검증(Phase 0식)이 필수라 릴리스 단위가 정본 (2026-07-13 결정) |
...
| 3.9 | README / 랜딩 문서 (Mac Whisper 대비 차별점: 완전 로컬, BT 퍼스트, 정돈 파이프라인) | |
| 3.10 | 베타: 지인 개발자 3–5명 온보딩 관찰 | 권한 플로우 이탈 관찰이 목적 |

**완료 기준**: 클린 맥(권한·모델 없음)에서 다운로드→온보딩→첫 주입까지 5분 내. 공증 통과. 자동 업데이트 동작.
