...
- **관찰 1 (2026-07-18)**: `brew tap`에서 `Error: git is unavailable` — 테스터 컴퓨터에 Xcode CLT 부재. 해결: `xcode-select --install`. README 설치 섹션에 트러블슈팅 추가. 시사점: "brew만 있으면 된다"는 가정이 깨짐 — CLT 없는 맥이 실재.
